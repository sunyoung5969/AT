...
$(function() {
 ...
 function fn_process(p_command){
	var v_url = '';
    var vp_id = $("#p_id").val();   			//  vp_id변수 선언 p_id로 초기화
    
    if(p_command == 'before'){
    	p_command = 'search';					// p_command search로 초기화
    	var before_id = $("#before_id").val();	// before_id에 before_id 값 저장
    	$("#p_id").val(before_id);  			// p_id에 before_id값 저장
    	vp_id = $("#p_id").val(); 				// vp_id에 p_id값 저장
    }
    
    if(p_command == 'after'){
    	p_command = 'search';					// p_command search로 초기화
    	var after_id = $("#after_id").val();	// after_id after_id 값 저장
    	$("#p_id").val(after_id); 				// p_id에 after_id 값 저장
    	vp_id = $("#p_id").val(); 				// vp_id에 p_id값 저장
    }    
